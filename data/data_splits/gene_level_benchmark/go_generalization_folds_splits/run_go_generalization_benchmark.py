...
def main():
    parser = argparse.ArgumentParser(
        description="task-term SVM evaluation with nested CV"
    )
    parser.add_argument(
        "--subfolder",
        required=True,
    ...
    parser.add_argument(
        "--cv-fold1-pkl", required=True, help="Path to task_cv_fold1_dict_all.pkl"
    )
    parser.add_argument(
        "--cv-fold2-pkl", required=True, help="Path to task_cv_fold2_dict_all.pkl"
    )
    parser.add_argument(
        "--cv-fold3-pkl", required=True, help="Path to task_cv_fold3_dict_all.pkl"
    )
    parser.add_argument(
        "--holdout-pkl", required=True, help="Path to task_holdout_dict.pkl"
    )
    parser.add_argument(
        "-d", "--out-root", required=True, help="Output directory to save results"
    )
    ...
    if len(txts) != 1:
        raise FileNotFoundError(
            f"Expected one .txt gene list in {subfolder}, found {len(txts)}"
        )
    ...
    for go_term in holdout_dict.keys():
        ...
        X_train_all, y_train_all = get_xy(
            pd.concat([fold1_df, fold2_df, fold3_df], ignore_index=True), embedding_df
        )
        ...
        if len(X_train_all) == 0 or len(X_holdout) == 0:
            ...
            holdout_auc_old = (
                roc_auc_score(y_holdout_out, y_prob_holdout_old)
                if len(np.unique(y_holdout_out)) > 1
                else np.nan
            )
            holdout_auprc_old = average_precision_score(
                y_holdout_out, y_prob_holdout_old
            )
        ...
        old_holdout_results_df = pd.DataFrame(old_holdout_records).set_index(
            "subfolder"
        )
    ...
    with open(
        "/results/single_gene/go_general/fold/" + name + "_go_post24_.pkl", "wb"
    ) as f:
        pickle.dump(all_fold_results, f)

    with open(
        "/results/single_gene/go_general/holdout/" + name + "_go_post24_.pkl", "wb"
    ) as f:
        pickle.dump(all_holdout_results, f)

    with open(
        "/results/single_gene/go_general/holdout_old/" + name + "_go_post24_.pkl", "wb"
    ) as f:

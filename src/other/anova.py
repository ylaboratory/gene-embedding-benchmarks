...
def main():
    ...
    ap.add_argument(
        "-i", "--input", required=True, help="Input CSV/TSV file of results."
    )
    ap.add_argument(
        "--sep", help="Separator (default auto: ',' for .csv, '\\t' for .tsv)."
    )
    ap.add_argument(
        "-d", "--dv", required=True, nargs="+", help="Dependent variable column(s)."
    )
    ap.add_argument(
        "-t",
        "--terms",
        default="Method + Category + Dimension",
    ...
    ap.add_argument(
        "-o", "--output", help="Output CSV path. Prints to stdout if omitted."
    )
    args = ap.parse_args()
